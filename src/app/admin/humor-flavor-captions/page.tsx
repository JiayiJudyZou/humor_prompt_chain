  ...
  return (
    ...
    >
      <div className="flex h-full min-h-0 flex-1 flex-col gap-5">
        ...
        <section className="min-h-0 flex-1 overflow-y-auto rounded-2xl border border-rose-100 bg-white/90 p-5 shadow-[0_10px_26px_rgba(15,23,42,0.06)] sm:p-6">
          <div className="mb-5 flex flex-wrap items-end justify-between gap-3 border-b border-rose-100 pb-4">
            <div>
              <h2 className="text-lg font-semibold text-slate-900 sm:text-xl">Flavors</h2>
              ...
              {flavors.map((flavor) => {
                ...
                return (
                  <li key={flavor.id} className="rounded-2xl">
                    ...
                    {isActive ? (
                      <div className="mt-2 rounded-xl border border-rose-200 bg-white/85 p-4 sm:p-5">
                        <div className="mb-4 flex items-center justify-between gap-3">
                          <p className="text-sm font-semibold text-slate-800">
                            Captions for {flavor.slug}
                          </p>
                          <p className="rounded-full border border-rose-100 bg-rose-50 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-600">
                            {captions.length} captions
                          </p>
                        </div>

                        {captions.length === 0 ? (
                          <div className="rounded-xl border border-dashed border-rose-200 bg-rose-50/70 p-5 text-center">
                          ...
                          <ul className="space-y-2.5">
                            {captions.map((captionRow, index) => {
                              ...
                              return (
                                <li
                                  key={captionRow.id}
                                  className="rounded-xl border border-rose-100 bg-gradient-to-r from-white to-rose-50/40 p-4 shadow-[0_6px_16px_rgba(15,23,42,0.04)]"
                                >
                                  <p className="text-xs font-semibold uppercase tracking-[0.1em] text-rose-500">
                                    Caption {index + 1}
                                  </p>
                                  <p className="mt-2 whitespace-pre-wrap text-[15px] leading-7 text-slate-900">
                                    {captionText ?? "No readable caption text field found."}
                                  </p>
                                  <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-slate-500">
                                    {formattedCreatedDate ? (
                                      <span className="rounded-full border border-rose-100 bg-white px-2 py-1">
                                        {formattedCreatedDate}
                                      </span>
                                    ) : null}
                                    <span className="rounded-full border border-rose-100 bg-white px-2 py-1">
